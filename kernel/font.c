 ...
 *
 * Based on Marcel Sondaar's 8x8 font derived from IBM's public domain
 * VGA character set. Only the printable range (0x20-0x7E) is listed;
 * control characters (0x00-0x1F) and DEL are all-zero via static
 * initialization. Each glyph is 8 rows of 8 pixels, least significant
 * bit first (bit 0 = leftmost pixel). */
